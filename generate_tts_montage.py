...
def read_image_files_from_folder(folder_path: str) -> list:
    """Function to parse folder containing images and concatenate into a list
...
def parse_set_image_folders(source_dir:str, set_code:str) -> None:
    """Function to parse default set image subfolders and create montages
    
    Parameters
    ------
    source_dir (str) -- path to root folder with default set image subfolders
...
def generate_image_montage(
    ...
    """Function to combine list of images into montages of a given grid size.
    ...
    montage_grid (tuple) -- tuple of (n_rows, n_cols) for
        montage image (default: (7, 10))
    montage_channel_axis (int) -- axis for colour information
        needed by scikit.util.montage (default: 3)
    """
    # establish maximum image count per montage grid
    # (less 1 to leave last card blank as per TTS requirements)
    ...
    for chunk_num, chunk_images in enumerate(chunked_images):
        image_montage = montage(
            chunk_images, grid_shape=montage_grid,
            channel_axis=montage_channel_axis
        )
        if len(chunked_images) > 1:
            iio.imwrite(
                f'{output_folder}/{output_filename}_{chunk_num}.jpg',
                image_montage
            )
        else:
            iio.imwrite(
                f'{output_folder}/{output_filename}.jpg', image_montage
            )
